...
pub fn calculate_stats(entries: &[FileEntry]) -> FileStats {
    ...
    for entry in entries {
        if entry.is_dir {
            ...
            let type_stats = stats.file_types.entry(file_type.to_string()).or_insert_with(TypeStats::new);
            type_stats.count += 1;
...
pub fn generate_summary_report(stats: &FileStats) -> SummaryReport {
    ...
    SummaryReport {
        ...
        median_file_size: percentiles.get(0).copied().unwrap_or(0),
        largest_file_size: stats.max_file_size,
        smallest_file_size: if stats.min_file_size == u64::MAX { 0 } else { stats.min_file_size },
        ...
        top_file_types: top_types.into_iter().map(|(name, stats)| (name.clone(), stats.clone())).collect(),
    }
}
...
mod tests {
    ...
    fn create_test_entry(name: &str, size: u64, is_dir: bool, file_type: Option<&str>) -> FileEntry {
        FileEntry {
